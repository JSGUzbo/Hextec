...
public class TinOreGenerator implements IWorldGenerator
{
	...
	public TinOreGenerator(Block block)
	{
		this.block = block;
	}
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,IChunkProvider chunkProvider) 
	{
		if(world.provider.getDimensionId() == 0)
		{
			...
			for(int i=0;i<25;i++)	//schleife fuer 30mal versuchen ore zu generieren
			{
				int posX = chunkX+random.nextInt(16);
				int posZ = chunkZ+random.nextInt(16);
				int posY = random.nextInt(50); //höhe auf der ore spawnt
